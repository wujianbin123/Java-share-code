...
//    }
    ...
    public signed_transaction transfer(String strFrom,
        ...
        tx.extensions = new HashSet<>();
        set_operation_fees(tx, get_global_properties().parameters.current_fees);
        Log.e("transfer_operation", accountObjectFrom.id + ",,,,,," + accountObjectTo.id + ",,," + transferOperation.amount);
...
//        forcastOperation.extensions = new HashSet<>();
        operations.operation_type operationType = new operations.operation_type();
...
//        tx.set_reference_block(dynamicGlobalPropertyObject.head_block_id);
        ...
        types.private_key_type privateKeyType = new types.private_key_type(MyApp.localWalletUser.getLocalActivePrivateKey());
        ...
        types.private_key_type privateKey = mHashMapPub2Priv.get(publicKeyType);
        if (privateKey != null) {
            tx.sign(privateKey, mWalletObject.chain_id);
        } else {
            Log.e("aaaaaaaaaaaaaaa", privateKey + "为null");
        }
